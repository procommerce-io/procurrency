...
string CRPCTable::help(string strCommand) const
{
    ...
    for (map<string, const CRPCCommand*>::const_iterator mi = mapCommands.begin(); mi != mapCommands.end(); ++mi)
    {
        ...
        if (fAllAnon)
        {
            if (strMethod != "sendproctotoken"
                && strMethod != "sendtokentotoken"
                && strMethod != "sendtokentoproc"
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "Stop ProCurrency server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "ProCurrency server stopping";
}
...
{ //  name                      actor (function)         okSafeMode threadSafe reqWallet
  //  ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "sendproctotoken",         &sendproctotoken,         false,     false,     false },
    { "sendtokentotoken",       &sendtokentotoken,       false,     false,     false },
    { "sendtokentoproc",         &sendtokentoproc,         false,     false,     false },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        std::string strWhatAmI = "To use procd";
