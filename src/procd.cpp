...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("ProCurrency version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  procd [options]                     " + "\n" +
                  "  procd [options] <command> [params]  " + _("Send command to -server or procd") + "\n" +
                  "  procd [options] help                " + _("List commands") + "\n" +
                  "  procd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "procurrency:"))
                fCommandLine = true;
