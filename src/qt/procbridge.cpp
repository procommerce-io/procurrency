...
bool UIProcBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 PROC, 1 ProCurrency
        switch(rcp.txnTypeInd)
        {
            case TXT_PROC_TO_PROC:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PROC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_PROC_TO_ANON:
                formatted.append(tr("<b>%1</b> to TOKEN %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PROC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to TOKEN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PROC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_PROC:
                formatted.append(tr("<b>%1</b>, ring size %2 to PROC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PROC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PROC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_AmountWithFeeExceedsTokenBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your TOKEN balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::PROC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
