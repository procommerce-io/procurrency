...
var qrcode = new QRCode("qrcode", {colorDark:'#0b1f37', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H, width: 220, height: 220,});
    ...
    check: function(field)
    {
        ...
        for (var i = 0, len = value.length; i < len; ++i)
            if (base58.base58Chars.indexOf(value[i]) == -1) {
                el.css("background", "#0b1f37").css("color", "white");
                return false;
            }
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="ProCurrency"                    ' + (unit.type == 0 ? "selected" : "") + '>PROC</option> \
                    <option value="1" title="Milli-ProCurrency (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mPROC</option> \
                    <option value="2" title="Micro-ProCurrency (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;PROC</option> \
                    <option value="3" title="Shi-ProCurrencys (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>sPROC</option> \
                </select> \
            </div> \
