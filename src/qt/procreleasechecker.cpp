...
#define paternVersion "proc-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

ProcReleaseChecker::ProcReleaseChecker(QObject *parent) : QObject(parent)
{
...
bool ProcReleaseChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> ProcReleaseChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(PROC_RELEASES)));
    QEventLoop event;
...
Version ProcReleaseChecker::getMaxReleaseVersion()
{
