...
#include "key.h"
#include "main.h"
#include "multisig/multisigaddressentry.h"
#include "multisig/multisiginputentry.h"
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                qint64 amount = recipient.amount;
...
{
    qint64 transactionSize = ui->signedTransaction->text().size() / 2;
    if(transactionSize == 0)
        return;

    // Check the fee
    qint64 fee = (qint64 ) (ui->fee->text().toDouble() * COIN);
    qint64 minFee = MIN_TX_FEE * (1 + (qint64) transactionSize / 1000);
    if(fee < minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 PROC) is smaller than the expected fee (%2 PROC). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
    else if(fee > minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 PROC) is bigger than the expected fee (%2 PROC). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
...
void MultisigDialog::updateAmounts()
{
    ...
    // Update Fee amount
    qint64 fee = inputsAmount - outputsAmount;
    QString feeStr;
