...
static const int MAX_OUTBOUND_CONNECTIONS = 60; //60 peers connection
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "ProCurrency " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. ProCurrency is probably already running."), addrBind.ToString());
        else
