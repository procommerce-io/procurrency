...
public:
    ...
    bool SendPROCToToken(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendTokenToPROC(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nPROC, int64_t& nToken) const
    {
        nPROC = 0;
        nToken = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable int64_t nCredPROCCached;
    ...
    bool GetCredit(int64_t& nCredPROC, int64_t& nCredToken, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredPROC = nCredToken = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredPROCCached = 0;
            nCredTokenCached = 0;
            pwallet->GetCredit(*this, nCredPROCCached, nCredTokenCached);
            fCreditSplitCached = true;
        };
        
        nCredPROC = nCredPROCCached;
